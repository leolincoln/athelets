...
def get_ind(result,names,name):
    ...
    try:
        ...
        sys.exit(1)
    return result[idx]
def get_individual(name,raw = None,p=2,plot=True,address=None,title=None):
    '''
...
def plot2(C=9999999):
    ...
    #for svm later
    for name in raw.keys():
        temp8910 = (raw[name][8]+raw[name][9]+raw[name][10])/3.0
        temp1112 = (raw[name][11]+raw[name][12])/2.0
        if len(name)>4:
            ath.append([temp8910,temp1112])
        else:
            control.append([temp8910,temp1112])
    ath = np.array(ath)
    control = np.array(control)
    f,ax = plt.subplots()
    ax.scatter(ath[:,0],ath[:,1],c='r',label='athlete')
    ax.scatter(control[:,0],control[:,1],c='b',label='control')
    X = np.concatenate((ath,control),axis=0)
    Y = np.array([0]*len(ath)+[1]*len(control))
    print X,Y
